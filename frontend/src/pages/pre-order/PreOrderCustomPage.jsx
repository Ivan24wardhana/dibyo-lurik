// =====================================================
// PreOrderCustomPage.jsx
// Fix: hapus import usePreOrderList (tidak exist).
// =====================================================

import { useState, useEffect, useRef, useCallback } from 'react'
import { Search, SlidersHorizontal, Eye, Pencil, Trash2, Sparkles } from 'lucide-react'
...
import DetailPOCModal from '../../components/pre-order/DetailPOCModal'
...
export default function PreOrderCustomPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/pre-order-custom?${params.toString()}`)
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/api/pre-order-custom/${po.id}`)
  ...
  return (
    <div>
      <div className="bg-[rgba(227,194,172,0.35)] border border-[#a47352] rounded-[10px] overflow-visible">
        ...
        <div className="px-6 py-4 flex items-center gap-3 flex-wrap border-b border-[#a47352]/30">
          ...
          <div className="relative flex-1 min-w-[280px]">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#a47352]/60 pointer-events-none" strokeWidth={2} />
            <input
              type="text"
              value={searchInput}
          ...
          <div className="relative" ref={filterContainerRef}>
            <button
              ...
              className={`
                ...
                hover:bg-[rgba(227,194,172,0.5)] active:scale-[0.98] transition-all
        ...
        <div className="overflow-x-auto">
          ...
          {loading ? (
            <div className="bg-white py-12"><Loading variant="centered" message="Memuat data pre-order..." /></div>
          ) : error ? (
            ...
            <div className="bg-white py-12">
              <EmptyState
                icon={Sparkles}
                title={searchQuery || activeFilterCount > 0 ? 'Tidak ada hasil' : 'Belum ada pre-order custom'}
                message={searchQuery ? `Tidak ditemukan dengan "${searchQuery}"` : 'List pre-order custom akan muncul di sini'}
              />
            </div>
          ) : (
            items.map((po, idx) => (
              <div
                ...
                className="
                  ...
                  hover:bg-[#fdfaf6] transition-colors min-w-[1050px]
                "
              >
                <div className="text-center text-[#a47352] text-sm font-medium">{startNum + idx}.</div>
                <div className="text-center text-[#a47352] text-sm font-medium">ID {po.id?.slice(-7)?.toUpperCase() || '-'}</div>
                <div className="text-center text-[#a47352] text-sm">{po.nama_customer || '-'}</div>
                <div className="text-center text-[#a47352] text-sm">{po.jumlah_item || '-'}</div>
                <div className="text-center text-[#a47352] text-sm">{fmtTanggal(po.tanggal_po)}</div>
                <div className="text-center text-[#a47352] text-sm">{fmtTanggal(po.tanggal_estimasi)}</div>
                <div className="flex justify-center"><StatusBadge value={po.status_produksi} /></div>
                <div className="flex justify-center gap-1.5">
                  <ActionBtn icon={Eye} label="Detail" color="#4cd0b1" onClick={() => setDetailModal({ open: true, id: po.id })} />
                  {(isCS || isKP) && (
                    <ActionBtn icon={Pencil} label="Edit" color="#f0a864" onClick={() => setDetailModal({ open: true, id: po.id })} />
                  )}
                  {isCS && (
                    <ActionBtn icon={Trash2} label="Hapus" color="#ff695e" onClick={() => setDeleteModal({ open: true, po })} />
                  )}
                </div>
              </div>
            ))
      ...
      {meta.totalPages > 1 && (
        <div className="mt-4">
          <Pagination page={page} totalPages={meta.totalPages} totalItems={meta.total} limit={10} onPageChange={setPage} />
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        title="Hapus Pre-Order Custom?"
        message={deleteModal.po ? `Pre-order dari "${deleteModal.po.nama_customer}" akan dihapus permanen. Lanjutkan?` : 'Apakah Anda yakin?'}
        confirmText="Ya, Hapus"
        loading={deleteLoading}
      />

      <SuccessPopup open={showDeleteSuccess} onClose={() => setShowDeleteSuccess(false)} message="Pre-order custom berhasil dihapus" duration={1500} />
    </div>
  )
}
...
function ActionBtn({ icon: Icon, label, color, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-white text-xs font-medium rounded-[8px] px-2.5 py-1.5 inline-flex flex-col items-center gap-0.5 min-w-[46px] active:scale-95 transition-all"
